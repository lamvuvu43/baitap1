(function ($) {
    ...
    function download_utf(filename, text) {
    ...
    function download_shi(filename, text) {
        const element = document.createElement("a");
        element.setAttribute(
            "href",
            "data:text/csv;charset=shift-jis,\ufeff" + encodeURIComponent(text)
        );
        element.setAttribute("download", filename);
    ...
    function convert(table) {
        ...
        const numCols = rows
            ...
            .find("td,th")
            .filter(":visible")
            .not(options.excludeColumns).length;

        rows.each(function (ignore, elem) {
            $(elem)
                .find("td,th")
                .filter(":visible")
                .not(options.excludeColumns)
                .each(function (i, col) {
                    const column = $(col);
    ...
    $.fn.table2csv_utf = function (action, opt) {
        ...
        switch (action) {
            case "download":
                download_utf(options.filename, csv);
                break;
            case "output":
            ...
            default:
                throw new Error(
                    '"action" argument must be one of the supported action strings'
                );
        }

        return this;
    };
    $.fn.table2csv_shi = function (action, opt) {
        if (typeof action === "object") {
            opt = action;
            action = "download";
        } else if (action === undefined) {
        ...
        switch (action) {
            case "download":
                download_shi(options.filename, csv);
                break;
            case "output":
                $(options.appendTo).append($("<pre>").text(csv));
                break;
